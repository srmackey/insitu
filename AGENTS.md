...
- A **article** is one markdown file under the user's vault `articles/`. Why-logs live under `provenance/<id>.md` and are not articles. Leftover `*.prov.md` under `articles/` is also not an article.
