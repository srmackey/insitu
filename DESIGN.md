...
Product name: **Insitu** (situated identity: who you are here). Working title during design was Protocol Vault. An **article** is a portable section of standing guidance. A **skill** is a procedure the host should expose as `/name`. A **protocol** is the assembled, project-specific "who I am here."
...
| Concept | Definition |
|---------|----------|
| **Article** | One portable section of standing agent guidance. A markdown file in the vault. Composed into a protocol. Ops slang: a config is made of articles. |
...
| **Pack** (0.9) | Versioned bundle of articles and roles, authored outside the vault. Installed copy lives under `library/<id>/<version>/`. Optional `skills/` on a pack is copied onto the shelf. `install_skill` (0.12) maps a listed skill; whole-capability install does not attach skills. |
| **Library** (0.9) | Vault shelf of pulled pack versions. Not native `articles/`. |
| **Import record** (0.9) | A project map entry: this node uses a whole **capability** or listed **articles** from pack id at `version` or `latest`. |
...
Folders under `articles/` (`interaction/`, `methodology/`, `knowledge/`) are a human convention. They are not a type system and do not change load behavior. **Roles** (`roles/*.yaml`) are a type system: they change composition. Do not treat an article folder as a role.
...
**Miss.** If no `projects/<folder>/` exists, `resolve_protocol` returns a structured miss. It does not scan the article catalog and it does not return an empty protocol that looks like success. The miss should name the project key that was tried and the path that was missing.
...
**Charset.** Project keys, article path segments, role ids, pack ids, and skill ids: `a-z`, `0-9`, `-`. Skill ids are a single path segment (no `skills/methodology/land/`). `_global` is the only reserved `_` name. Reject `..`, absolute paths, and anything that would escape `articles/`, `provenance/`, `projects/`, `roles/`, `library/`, or `skills/`.
...
imports:                          # optional; 0.9 capability / article installs
  - pack: system-development
    version: 0.1.0                # or latest; omit members = whole capability
  # - pack: voices
  #   version: 1.1
  #   articles: [identity/x]       # 0.13: core members
...
A **role** is a named, ordered pack of articles. It is how a kind of project (clerk, steward, archivist) carries a shared set of rules without listing every member on every map, and without stuffing `_global`.

Roles are vault content, not server builtins. The server has no built-in role names.

**Why not a tag scan.** Article frontmatter cannot be the membership source: injection order must be explicit and stable, and a `roles:` tag must not silently enlarge every node protocol. The role file is the membership list, and it is the only copy (0.16). An article carrying a stale `roles:` key from an older vault is ignored on load, never read, and never rewritten.
...
**Article frontmatter.** An article does not declare its roles (0.16). Membership is a fact about the role file, and an article is a member of it the way a song is in a playlist: the playlist knows.

Before 0.16 the role id was mirrored into article frontmatter and `validate` checked the two against each other. Composition never read that copy, `update_role` machine-wrote it, and `validate --fix` repaired it in one direction only, which is the design already naming the role file authoritative. A check between a source and its own cache can only report that the cache is stale; it cannot catch a real error. Removed, along with the `role` filter on `list_articles` that was its one consumer. `get_role` answers membership from the file that owns it.

**Project includes.** `map.yaml` `roles:` is an ordered list of role ids. A project may also list articles in `core` / `on_demand` as today. First occurrence wins when the same article appears in a role and again on the map.
...
`expand(roles)` walks the project's `roles:` list in order and concatenates each role's `core` (or `on_demand`). Unknown role id is a hard error, same as a missing article. Later protocol entries still refine earlier ones, so `project.core` wins over a role, and a role wins over `_global`.
...
**Composition.** `expand(imports)` walks map records in order. Resolve `latest` first. Whole capability (no `articles` / `skills` keys): expand that version’s `roles/<pack>.yaml` like a native role. Article members: inject only those article files from that version. A record's `articles:` are core members and its `on_demand:` are indexed rather than injected, the same split a pack role expresses for a whole capability (0.13). Skill members (`skills:` on the record): compose those pack skills after native `project.skills`. Same article or skill id from two records on one project is a hard error.
...
Native `roles:` never searches `library/`. `imports:` never searches vault `roles/`. `get_article` looks native `articles/` first, then this project’s import records at their versions.
...
- Every article id inside a role file exists.
- **Membership match:** if `roles/<id>.yaml` lists article S, S's frontmatter must include `id` in `roles:`. If S's frontmatter lists a role, that role file must list S (in `core` or `on_demand`). Mismatches are reported. `fix=true` writes the missing frontmatter role onto the article; it does **not** add an article to a role file (that would change every project using the role).
- Duplicate article ids inside one role file, and duplicates created only by expansion, are reported the same way as map dupes. Expansion itself already drops later copies.
- **Packs (0.9):** each exact `imports:` record has `library/<pack>/<version>/` on disk; listed `articles:` and `on_demand:` members exist in that tree; `unreferenced_version` is a finding (`fix` does not delete); broken exact pin is a hard error. Same article id from two import records on one project is a hard error.

**`where_used`.** An article is used by: project maps that list it in `core` or `on_demand`; role files that list it; project maps whose `roles:` or `imports:` expand to include it (report `lists: [role:<id>]` or `lists: [import:<pack>@<version>]`).

**Tools.** `list_roles` and `get_role` are authoring tools (parallel to `list_projects` / `get_project`). `get_role` returns the role file, member sizes, and which projects include it. `resolve_protocol` reports the role ids that were expanded (`roles: [node]`). `list_articles` does not filter by role: membership is `get_role`. `link_article` / `unlink_article` stay one article on one project map. Role membership is `create_role` / `update_role` (add/remove on `core` and `on_demand` is preview then `confirm=true`). `delete_role` is user-gated the same way.

**Not a mode.** A role is composition, not a runtime hat. Including `node` means those articles are in the protocol. It does not switch Architect vs Maintainer or otherwise change tool privilege.
...
Those article files are authored in a vault. The role *mechanism* ships before any of them exist.
...
**The calling chair** is the basename of `working_folder`, case-folded. All thirteen mutating map tools require it: `link_article`, `unlink_article`, `link_skill`, `unlink_skill`, `install_capability`, `uninstall_capability`, `install_article`, `uninstall_article`, `install_skill`, `uninstall_skill`, `create_project`, `update_project`, `delete_project`. A bound chair whose key is not `project` gets `chair_bound`. `materialize` already took `working_folder` and joins the same gate; naming no project means this folder's own map, which is always allowed.

**`materialize` reads `working_folder` twice (0.15.1).** For the gate it is the calling chair, and for the write it is the destination. Those are the same folder for a bound chair, because the gate requires the basename to equal `project`. An admin is waved past that comparison, so nothing else stopped an admin from writing one project's protocol over another project's checkout, and the skill prune (§10) would then delete the generated skills it found there. A named `project` must therefore match the destination folder's basename, for every class and in a pre-init vault: mismatch is `folder_project_mismatch` and nothing is written, not even the folder. A sweep names each project's own checkout, so correct usage never meets this refusal. The project key is defined as the folder basename (§4), which is what makes a mismatch an error rather than a preference.

**Reach, not ownership (0.15).** An article, role, or skill belongs to no single map, so binding it to one key would be wrong. What is gated instead is reach: a write that changes what a map other than the calling chair composes is composition authority, and that is admin. The ten writers of shared objects therefore also take `working_folder`: `create_article`, `update_article`, `delete_article`, `create_role`, `update_role`, `delete_role`, `create_skill`, `update_skill`, `delete_skill`, and `validate` when `fix=true`.
...
Reach is measured as composition, not as a direct link: an article pulled in through a role counts, and an article on `_global` reaches everything. `validate --fix` rewrites shared vault files, so it takes the same gate against every map in the vault.
...
- `id` must match the path relative to `articles/` (no `.md`).
...
- **Order is preserved** (`_global` composed first, then each listed role, then imports, then project `core`). Order is injection order. Where guidance conflicts, later entries refine or override earlier ones, so project-specific articles take precedence over roles and imports, and roles take precedence over `_global`.
- **Duplicates:** if an article appears more than once across `_global`, roles, imports, and `project.core`, the first occurrence wins and later occurrences are dropped. `validate` reports duplicates; it removes them only with `fix=true` (map-local copies only; role files are not rewritten by `fix`).
...
- **Missing article or role references are a hard error.** If any map, including `_global`, or any role file references an article that does not exist, or a map names a role that does not exist, resolution fails with an explicit error naming the broken reference. Determinism over convenience.
- **An unparseable file is a hard error naming that file.** A malformed article, skill, role, map, lock, pack, or `config/` file fails with its own path and the parse position (`VaultReadError`), wherever it is read. One bad file is never skipped silently, and the caller never gets a bare parser message that does not say which file it came from. Pack trees count: content Insitu did not author still gets named, not swallowed. Same principle as a broken reference.
...
- On-demand articles are **not** included in the protocol content. The resolved protocol **does include an index** of `expand(project.roles).on_demand` plus `expand(project.imports).on_demand` plus the project's `on_demand` list (id, title, description, size) so the agent knows what it can request via `get_article`.
- **Skills index (0.11 / 0.12).** After the on-demand index, `resolve_protocol` lists composed skills (`id`, `name`, `description`, size): native `project.skills` then pack import `skills:` lists. No skill `content`. Skill text is never injected into `core`. Missing skill id on a map is a hard error (`missing_skill`), same as a missing article. Duplicate native-and-pack or two-pack ids is `duplicate_import_skill`. Size totals stay article-only; `skills_size` (count, bytes, estimated tokens) is a separate skill-index summary.
- The resolved protocol reports **size metadata** on the result *before* the core bodies: article count, total bytes, and estimated tokens (`chars / 4`, labeled an estimate), plus the same fields on each core article. Size is an authoring surface, not just telemetry: a user (or an agent helping them) should be able to see protocol weight and trim articles before they bloat every session.
...
| Tool | Description |
|------|-------------|
| `resolve_protocol` | Return the project protocol per §8: ordered core articles (content + metadata), the `on_demand` index, **the skills index**, **expanded `roles`**, and size totals. **Live / inspect tool:** weigh the composition, refresh mid-session, compare against a materialized header. Not how core guidance enters the session. |
| `get_article` | Return a single article (content + metadata, including size/tokens) by path/id. |
| `list_articles` | List all articles in the vault with metadata including title, description, tags, **size and estimated tokens**. Optional path prefix / tag filters. **User bootstrapping / authoring tool:** a user in a new project lists articles to decide what to `link_article`, and uses the size fields to see what needs editing. Not agent-session bootstrap. |
...
| `get_project` | Return a project's map, notes, **roles**, **skills**, a **protocol size summary** (article count, total size, estimated tokens), and a **skills size summary**, without returning protocol content. Authoring check: "how heavy is this project's protocol?" |
| `project_status` | **0.10.** Folder inspect card. Required `working_folder`, optional `project` (default: folder basename). Map, sourced core ids, composed size, on-demand ids, disk freshness, plus a `card` string. No article bodies. Inspect only: never writes. Not session start. |
...
| `get_role` | Return one role file, member article metadata and sizes, and the project keys that include it. |
| `list_on_demand` | List the on-demand articles associated with a project (id, title, description, size). |
| `link_article` | Add an article to a project's `core` or `on_demand` list (subject to review policy). Writes immediately and reports `affects_projects`. |
| `unlink_article` | Remove an article from a project's map (subject to review policy). Writes immediately and reports `affects_projects`. |
| `create_article` | Create a new article (subject to review policy). Does not auto-link. `affects_projects` is empty. |
| `update_article` | Update an existing article (subject to review policy). Surfaces `where_used` and `affects_projects` so the blast radius is visible. |
| `delete_article` | User-gated delete. Preview (no write) unless `confirm=true` with the preview's `expected`. Unlinks from role files and direct maps, then deletes the article and its why-log. |
| `create_role` | Create `roles/<id>.yaml`. New role is on no project. `affects_projects` is empty. |
| `update_role` | Incremental. Name/description write now. Member add/remove is preview unless `confirm=true` with `expected`; confirm writes the role file and article `roles:` frontmatter. Maps do not change. |
...
| `delete_project` | User-gated delete of `projects/<key>/` only. Articles and roles stay. `_global` cannot be deleted (`cannot_delete_global`). |
| `where_used` | List every project map (core, on-demand, or via a role) and every role file that references a given article. |
| `validate` | Vault health check: verifies every referenced article and role exists, ids match paths, required frontmatter is present, and reports duplicate map entries. Read-only by default (safe to run any time). `fix=true` applies safe repairs (drop duplicate map entries) and follows the review dial. **Findings** (`empty_projects`, `empty_roles`, `unreferenced`, `not_in_any_protocol`) are authoring hygiene: they never fail `ok` and `fix` never consumes them. |
| `materialize` | **Enforcement path.** Resolve per §8 and write `PROTOCOL.md` plus one generated file per enabled host surface (§10), then generated skill copies for each mapped skill. Never writes `AGENTS.md`, `CLAUDE.md`, or `CLAUDE.local.md`. Files carry a generated-file header (vault root, vault git ref if any, timestamp, project key, article ids in order) for staleness detection. Skill copies stamp after frontmatter. A named `project` must match the destination folder's basename, or the call is refused with `folder_project_mismatch` and writes nothing (§6.3). |
...
| `install_article` | **0.9.** This project uses one article from a pack version. User-facing: “install identity x at 1.1.” **0.13:** `target` is `core` (default) or `on_demand`, mirroring `link_article`. |
...
| `uninstall_capability` / `uninstall_article` / `uninstall_skill` | **0.9 / 0.12.** Drop this map’s record (or that member) only. Shelf unchanged. |
...
Native vault skills use `link_skill`. Pack skills use `install_skill`. `link_article(..., target="skills")` remains `invalid_target`.
...
Deliberately absent: `rename_article` / `rename_role` / `rename_project`, which are not in v1. Registering the first admin is not a tool; it is `insitu init --admin` on the command line.
...
- Insitu stores reusable articles and composes a project protocol.
...
- `materialize` writes only the generated pack. It never writes `AGENTS.md` or `CLAUDE.md`. If other host files this host loads are missing, retrieve the multi-platform pack (`get_article` `platform/router-vs-pack` and this host's profile) and write them here. Do not put pack bodies in the router. If those articles miss, the pack is not installed; say so. Do not invent files the pack does not name.
...
- Pull on-demand articles with `get_article` when the work needs one. Do not pull the on-demand list at session start.
- Use `list_articles` to see the catalog and sizes (what to include, what to trim). Do not call it at session start to load the core.
...
- `get_article` / `list_on_demand`: on-demand extras
...
- **Article why-log:** `provenance/<id>.md`, same path as the article id. Append-only markdown. Written on `create_article` and `update_article` (and by hand). The article file remains the source of truth; the why-log is the rationale. No yaml. No `.prov.md` suffix (the folder is the type).
- **Leftover siblings.** A pre-0.7 `articles/<path>/<stem>.prov.md` is not an article. The next `update_article` or `delete_article` migrates or removes it. Do not author new siblings.
...
- **Frontmatter** `created` / `updated` / `author` on the article remain optional metadata, not a substitute for the why-log.
...
- **Weight is visible** — article sizes and composed-protocol sizes are first-class authoring data. A protocol that cannot be weighed will grow.
