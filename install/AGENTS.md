...
- Pull on-demand articles with `get_article` when the work needs one. Do not pull the on-demand list at session start. Use `list_articles` to see the catalog and sizes (what to include, what to trim). Do not call it at session start to load the core.
