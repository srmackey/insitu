...
`materialize` writes only that generated pack. It never writes `AGENTS.md` or `CLAUDE.md`. If other host files this host loads are missing, retrieve the multi-platform pack (`get_article` `platform/router-vs-pack` and `platform/claude-code`) and write them here. Do not put pack bodies in this router. If those articles miss, the pack is not installed; say so. Do not invent files the pack does not name.
